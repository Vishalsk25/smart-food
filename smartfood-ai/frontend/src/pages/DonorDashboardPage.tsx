...
export default function DonorDashboardPage() {
  ...
  const [pickupLocation, setPickupLocation] = useState<MapLocation | undefined>();
  ...
  const loadDonations = () => {
    ...
    setMyDonations(allDonations.filter((d: any) => d.donorName?.toLowerCase() === userProfile.name.toLowerCase() || d.donorName === 'Donor Name'));
  };

  useEffect(() => {
    loadDonations();
    const handleStorage = () => loadDonations();
            ...
            <div className="dd-card fade-in">
              ...
              <form className="dd-form-grid">
                ...
                <div className="dd-form-group" style={{gridColumn: '1 / -1'}}>
                  ...
                  <p style={{ color: '#94a3b8', fontSize: '0.9rem', marginBottom: '0.5rem' }}>Click on the map to pin the exact pickup location.</p>
                  <LiveMapTracker 
                    mode="picker" 
                  ...
                  {pickupLocation && (
                    <div style={{ marginTop: '0.5rem', color: '#38bdf8', fontSize: '0.9rem' }}>
                      Selected: Lat {pickupLocation.lat.toFixed(4)}, Lng {pickupLocation.lng.toFixed(4)}
                    </div>
                  )}
                </div>
                    ...
                    {['Accepted', 'On the Way to Pickup', 'Picked Up', 'On the Way to Delivery'].includes(donation.status) && (
                      <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid #1e293b', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                        ...
                        <div style={{ width: '100%', height: '100%', minHeight: '200px', backgroundColor: '#0f172a', borderRadius: '12px', border: '1px solid #1e293b', overflow: 'hidden' }}>
                          <LiveMapTracker 
                            mode="tracking"
                            pickupLocation={{ lat: donation.pickupLat || 12.9716, lng: donation.pickupLng || 77.5946 }}
                            dropoffLocation={{ lat: donation.dropLat || 12.9916, lng: donation.dropLng || 77.6146 }}
                            driverLocation={{ lat: donation.ngoLat || 12.9750, lng: donation.ngoLng || 77.6000 }}
