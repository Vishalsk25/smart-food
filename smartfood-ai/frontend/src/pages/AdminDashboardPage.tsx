...
export default function AdminDashboardPage() {
  ...
  const donationIntents = useMemo(() => readLocalArray<DonationIntentRecord>('donationIntents'), [activeSection, syncMessage]);
  const donateMeUsers = useMemo(() => readLocalArray<any>('donateMeUsers'), [activeSection, syncMessage]);
  const foodRequests = useMemo(() => readLocalArray<any>('foodRequests'), [activeSection, syncMessage, adminMessage]);
  const adminNotifications = useMemo(() => readLocalArray<NotificationRecord>(ADMIN_NOTIFICATIONS_KEY), [activeSection, syncMessage]);
      ...
      <section className="admin-panel" style={{ marginTop: '18px' }}>
        ...
        {activeSection === 'liveTracking' && (
          <section style={{ marginTop: '22px', display: 'grid', gap: 14 }}>
            ...
            <div style={{ width: '100%', height: '400px', backgroundColor: '#0f172a', borderRadius: '12px', border: '1px solid #1e293b', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#94a3b8', position: 'relative', overflow: 'hidden', marginBottom: '1rem' }}>
              <div style={{ position: 'absolute', inset: 0, opacity: 0.1, backgroundImage: 'radial-gradient(#38bdf8 1px, transparent 1px)', backgroundSize: '30px 30px' }}></div>
              <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#38bdf8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: '1rem', zIndex: 1 }}>
                <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path>
                <circle cx="12" cy="10" r="3"></circle>
              </svg>
              <h2 style={{ margin: 0, color: '#f8fafc', zIndex: 1 }}>Global Live Map Tracking</h2>
              <p style={{ margin: '0.5rem 0 0', zIndex: 1 }}>Monitoring active deliveries across the network in real-time.</p>
            </div>
