...
export default function AshramaDashboardPage() {
  ...
  const [requestLocation, setRequestLocation] = useState<MapLocation | undefined>();
  ...
  const loadIncomingDeliveries = () => {
    try {
      const all = JSON.parse(localStorage.getItem('donationIntents') || '[]');
      const filtered = Array.isArray(all) ? all.filter((d: any) => d.toLocation?.toLowerCase().includes(sessionUser?.fullName?.toLowerCase() || '') || d.status !== 'Pending') : [];
      setIncomingDeliveries(filtered);
    } catch { 
  ...
  useEffect(() => {
    if (sessionUser) {
      ...
      const handleStorage = () => loadIncomingDeliveries();
          ...
          <div className="ngo-section ngo-card" style={{ maxWidth: '800px', margin: '0 auto' }}>
            ...
            <form onSubmit={handleRequestSubmit}>
              ...
              <div className="ngo-form-group">
                ...
                <p style={{ color: '#94a3b8', fontSize: '0.9rem', marginTop: '1rem', marginBottom: '0.5rem' }}>Click on the map to pin your exact delivery location.</p>
                <LiveMapTracker 
                  mode="picker" 
                  onLocationSelect={setRequestLocation} 
                ...
                {requestLocation && (
                  <div style={{ marginTop: '0.5rem', color: '#38bdf8', fontSize: '0.9rem' }}>
                    Selected: Lat {requestLocation.lat.toFixed(4)}, Lng {requestLocation.lng.toFixed(4)}
                  </div>
                )}
              </div>
                ...
                <div key={req.id} className="ngo-list-item" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: '0.75rem' }}>
                  ...
                  {['Accepted', 'On the Way to Pickup', 'Picked Up', 'On the Way to Delivery'].includes(req.status) && (
                    <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid #1e293b', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', width: '100%' }}>
                      ...
                      <div style={{ width: '100%', height: '100%', minHeight: '200px', backgroundColor: '#0f172a', borderRadius: '12px', border: '1px solid #1e293b', overflow: 'hidden' }}>
                        <LiveMapTracker 
                          mode="tracking"
                          pickupLocation={{ lat: req.pickupLat || 12.9716, lng: req.pickupLng || 77.5946 }}
                          dropoffLocation={{ lat: req.dropLat || 12.9916, lng: req.dropLng || 77.6146 }}
                          driverLocation={{ lat: req.ngoLat || 12.9750, lng: req.ngoLng || 77.6000 }}
