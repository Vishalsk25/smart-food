...
public class DonationController {
    ...
    public DonationController(FoodDonationService donationService) {
        this.donationService = donationService;
    }

    @PostMapping
    public ResponseEntity<ApiResponse<FoodDonationDTO>> createDonation(
        @RequestParam Long donorId, @RequestBody FoodDonationCreateDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED)
            .body(ApiResponse.success("Donation created successfully", donationService.createDonation(donorId, dto)));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<FoodDonationDTO>> updateDonation(
        @PathVariable Long id, @RequestBody FoodDonationUpdateDTO dto) {
        return ResponseEntity.ok(ApiResponse.success("Donation updated successfully", donationService.updateDonation(id, dto)));
    }
    ...
    public ResponseEntity<ApiResponse<List<FoodDonationDTO>>> getNearbyDonations(
        ...
        return ResponseEntity.ok(ApiResponse.success(donationService.getNearbyDonations(latitude, longitude, radiusKm)));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse<Void>> deleteDonation(@PathVariable Long id) {
        donationService.deleteDonation(id);
        return ResponseEntity.ok(ApiResponse.<Void>success("Donation deleted successfully", null));
    }
}
